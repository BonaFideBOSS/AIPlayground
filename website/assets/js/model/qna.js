const predict = async () => {
  ...
  document.querySelector(".status-modal .modal-body").innerHTML =
    '<p class="fs-5 my-0">Model loaded successfully!</p>';
  modal.hide();
  ...
  document.getElementById("search").addEventListener("submit", (e) => {
    ...
    model.findAnswers(question.value, context.value).then((answers) => {
      ...
      if (answers.length > 0) {
        ...
        result.innerHTML +=
          context.value == ""
            ? bot_msg("Context not provided.")
            : bot_msg("Answer not found.");
      }
