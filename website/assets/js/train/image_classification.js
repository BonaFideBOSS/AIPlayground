...
async function trainAndPredict() {
    ...
    let results = await model.fit(inputsAsTensor, oneHotOutputs, {
        shuffle: true, batchSize: 5, epochs: EPOCHS,
...
function predictLoop() {
    if (predict) {
        tf.tidy(function () {
            ...
            STATUS.innerHTML = `Prediction: <span class="fw-semibold">${CLASS_NAMES[highestIndex]}</span> with <span class="fw-semibold">${Math.floor(predictionArray[highestIndex] * 100)}%</span> confidence`;
        });
...
function logProgress(epoch, logs) {
    var percentage = (epoch + 1) * 100 / EPOCHS
    $(STATUS).find('p').text('Training... please wait. This may take several minutes.')
...
function get_classes() {
    ...
    for (let i = 0; i < dataCollectorButtons.length; i++) {
        ...
        var class_name = $(dataCollectorButtons[i]).parent().parent().find('input').val()
...
function dataGatherLoop() {
    if (videoPlaying && gatherDataState !== STOP_DATA_GATHER) {
        ...
        for (let n = 0; n < CLASS_NAMES.length; n++) {
            ...
            if (samples > 0) {
                $(`[data-image-samples="${n}"]`).text(`Image samples: ${samples}`)
            }
        }
...
add_class_btn.addEventListener('click', function () {
    ...
    <div class="classes card rounded-3">
        <div class="card-header">
            <input type="text" placeholder="class name" value="Class ${no_of_classes + 1}"
                class="form-control form-control-lg">
        </div>
        <div class="card-body">
            <p class="sample-count" data-image-samples="${no_of_classes}">Add image samples:</p>
            <button class="dataCollector btn btn-outline-primary" data-1hot="${no_of_classes}" data-name="Class ${no_of_classes + 1}" ${disabled}>
                Capture images
            </button>
        </div>
    </div>`
